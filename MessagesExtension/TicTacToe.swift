...
enum TTTError: Error, CustomStringConvertible {
...
class TicTacToe {
    ...
    private func isDraw() -> Bool {
        var d = Decimal()
        return moveCount(for: player!) == Int(size * size - 1)
    }
...
extension TicTacToe {
    func boardToJSON() -> String? {
        do {
            let data = try JSONSerialization.data(withJSONObject: self.grid.map({ (value: [TTTCellState]) -> [String] in
                var array = [String]()
                for i in 0..<value.count {
                    array.append(String(describing: value[i]))
                }
...
extension TicTacToe {
    ...
    convenience init?(queryItems: [URLQueryItem], current uuid: String) {
        ...
        self.init(player: current!, opponents: opponents!, size: TicTacToe.boardFrom(json: queryItems[1].value!)!.count)
    }
}
